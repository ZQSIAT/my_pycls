...
def train_epoch(loader, model, ema, loss_fun, optimizer, scaler, meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels) in enumerate(loader):
        ...
        with amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            criterion_func_string = "loss_fun.{:}(outputs=preds, targets=labels, sigma={:}, lamda={:}," \
                                    " theta={:}, top_c={:})".format(cfg.MODEL.lossfn, cfg.MODEL.sigma, cfg.MODEL.lamda,
                                                                    cfg.MODEL.theta, cfg.MODEL.topc)

            loss = eval(criterion_func_string)
            pass
        # todo: Be commented
        # print("I am here,trainer.py #106 and loss is {:}".format(loss))
        # exit()
...
def train_model():
    # print("train.py #161 cfg.MODEL.topc: {:}".format(cfg.MODEL.topc))
    """Trains the model."""
    ...
    method_list = [func for func in dir(loss_fun) if
                   callable(getattr(loss_fun, func)) and func.startswith("Compute")]
    assert cfg.MODEL.lossfn in method_list, "The loss functions should be in \'{:}\'.".format(method_list)
    # print("I am here:\n{:}\n".format('trainer.py #161 model = setup_model()'))
    # todo: commented
    # os.environ['CUDA_VISIBLE_DEVICES'] = '9'
    # print("####" * 25, '\n', 'Start\n')
    # preds = 0.1 * torch.ones(128, 10).cuda()
    # # y_train = torch.randn(128, 10).cuda()
    # labels_one_hot = torch.empty(128, dtype=torch.long).random_(10).cuda()  # torch.randn(128, dtype=torch.long).cuda()
    # criterion_func_string = "loss_fun.{:}(outputs=preds, targets=labels_one_hot, sigma={:}, lamda={:}, theta={:}, top_c={:})".format(
    #     cfg.MODEL.lossfn, cfg.MODEL.sigma, cfg.MODEL.lamda, cfg.MODEL.theta, cfg.MODEL.topc)
    # print(criterion_func_string)
    # loss = eval(criterion_func_string)

...
_NAME_PREFIX = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()) + "_model_epoch_"
...
def get_checkpoint_best():
    ...
    return os.path.join(cfg.OUT_DIR, time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())+"_model.pyth")


def get_last_checkpoint():
    """Retrieves the most recent checkpoint (highest epoch number)."""
    checkpoint_dir = get_checkpoint_dir()
    checkpoints = [f for f in pathmgr.ls(checkpoint_dir) if _NAME_PREFIX.split('_')[-1] in f]
...
def has_checkpoint():
    ...
    if not pathmgr.exists(checkpoint_dir):
        return False
    return any(_NAME_PREFIX.split('_')[-1] in f for f in pathmgr.ls(checkpoint_dir))

...
# _C.MODEL.LOSS_FUN = "cross_entropy"  # Default option
_C.MODEL.LOSS_FUN = "CELossAndHLossAndFLoss"  # My loss class: CELossAndHLossAndFLoss
_C.MODEL.lossfn = "ComputeCELoss"
_C.MODEL.sigma = 0.02  # Discrimination Term：Multi Variate Normal
_C.MODEL.lamda = 0.001  # Discrimination Term: lamda is a trade-off parameter
_C.MODEL.theta = 0.01  # Fidelity Term：the corresponding elements in the predicted label vector should be zeros.
_C.MODEL.topc = 3  # Fidelity Term：Select the topc category as a fuzzy label
...
def load_cfg_fom_args(description="Config file options."):
    ...
    parser.add_argument("--cfg",
                        default="/home/432/zhaoqingsong/code/my_pycls/configs/dds_baselines/resnet/R-50-1x64_dds_8gpu_lossfn.yaml",
                        dest="cfg_file",
                        help=help_s,
                        required=False,
                        type=str)
    ...
    # print(sys.argv)  # The params number of cmd
    if len(sys.argv) == 1:
        parser.print_help()
        # sys.exit(1)
    args = parser.parse_args()
    load_cfg(args.cfg_file)
    # print("config.py #434 cfg.MODEL.lossfn: {:}".format(cfg.MODEL.lossfn))
